...
from envs.env_tool import check1, get_shape_from_obs_space
from hasac_models.base.cnn import CNNBase
from hasac_models.base.mlp import MLPBase
from hasac_models.base.act import ACTLayer
...
class StochasticMlpPolicy(nn.Module):
    ...
    def forward(self, obs, available_actions=None, stochastic=True):
        ...
        """
        obs = check1(obs).to(**self.tpdv)
        deterministic = not stochastic
        if available_actions is not None:
            available_actions = check1(available_actions).to(**self.tpdv)
