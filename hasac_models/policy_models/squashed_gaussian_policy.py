...
from envs.env_tool import get_shape_from_obs_space
from hasac_models.base.plain_cnn import PlainCNN
from hasac_models.base.plain_mlp import PlainMLP
...
class SquashedGaussianPolicy(nn.Module):
    ...
    def forward(self, obs, stochastic=True, with_logprob=True):
        ...
        # print(mu)
        # print(std)
        # mu = torch.where(torch.isnan(mu), torch.ones_like(mu), mu)#避免出现nan，赋值为0
        # std= torch.where(torch.isnan(std), torch.ones_like(std), std)#避免出现nan，赋值为0
        #这个地方跑mpe有问题，会出现Nan
