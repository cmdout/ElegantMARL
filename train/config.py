...
class Config:
    def __init__(self, agent_class=None, env_class=None, env_args=None):
        ...
        self.obs_dim = env_args['obs_dim']  # vector dimension (feature number) of obs
        self.agent_num = env_args['agent_num']  # agent number of env
        self.action_spaces = env_args['action_spaces']
        ...
        if self.if_off_policy:  # off-policy
            ...
            self.buffer_size = int(2048)  # ReplayBuffer size. Empty the ReplayBuffer for on-policy.
        ...
        self.num_workers = env_args['num_workers']  # rollout workers number pre GPU (adjust it to get high GPU usage)
...
def get_mpe_env_args(args, env, if_print: bool) -> dict:
    env_name = getattr(env, 'env_name', 'env')
    max_step = getattr(env, 'max_step', 12345)
    state_dim = env.state_dim
    action_dim = env.action_dim
